...
for i, item in enumerate(total_average_KD):
    ...
    #parsing
    this_protein_id=this_protein[0]
    this_protein_header = this_protein[1]
    this_TMD_KD = this_protein[2]
    that_protein = total_average_KD[i+1]

    if this_protein[0] == that_protein[0]:
        that_TMD_KD = that_protein[2]
        #print(this_TMD_KD, "and", that_TMD_KD)
        this_that_pair=[this_TMD_KD, that_TMD_KD]
        this_TMD_pair_KD=numpy.mean(this_that_pair)
        output = this_protein_id + ", " +  str(this_TMD_pair_KD)
        output.replace('(','')
        output.replace(')',',')
        output.replace("'","")
        print(output)

    elif this_protein[0] != that_protein[0]:
        pass
