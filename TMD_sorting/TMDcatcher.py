...
filenames = ["mouse_human_TRANSMEM.txt"]
...
#Nflank output_filename

input_format = "swiss"
feature_type = "TRANSMEM"
output_filename = "N_flanking.fasta"
...
for filename in filenames:
    ...
    for record in SeqIO.parse(filename, input_format): #A biopython module that can automatically parse uniprot .txt files
        for f in record.features:
            #feature type refers to TRANSMEM
            if f.type == feature_type:
                #These are the flanking regions and the TMD using record.seq
                flank1 = record.seq[(f.location.start-5):(f.location.start)]
                flank2 = record.seq[(f.location.end):(f.location.end+5)]
                TMD = f.extract(record.seq)
                flank1length = len(flank1)
                flank2length = len(flank2)
                #Here we define the ID code and then the start and end positions.
                title_line = "'%s' | N-terminal flank: '%i-%i'" % (record.id, f.location.start-flank1length, f.location.start)
...
input_format = "swiss"
feature_type = "TRANSMEM"
output_filename = "TMH_only.fasta"
...
input_format = "swiss" #This SHOULD work with uniprot filetype
feature_type = "TRANSMEM"
output_filename = "C_flanking.fasta"
    ...
    for record in SeqIO.parse(filename, input_format): #A biopython module that can automatically parse uniprot .txt files
        for f in record.features:
            ...
            if f.type == feature_type:
                ...
                flank2length = len(flank2)
                #Here we define the ID code and then the start and end positions.
                title_line = "'%s' | C-terminal flank: '%i-%i'" % (record.id, f.location.end+1, f.location.end+flank2length+1)
